  ...
  <AppLayout>
    ...
    <div class="page-top">
      ...
      <div style="display: flex; gap: 12px; align-items: center;">
        <button class="btn-primary-action btn-red-action" @click="openReturnModal">
    ...
    <div class="metrics-cards-grid">
      ...
      <div class="card-metric card-amber font-hero">
        ...
        <div>
          ...
          <h4 class="metric-val text-amber">{{ formatMoney(metrics.total) }} so'm</h4>
        </div>
      </div>
    </div>
        ...
        <table v-else class="data-table">
          ...
          <tbody>
            <tr v-for="p in payments" :key="p.id" class="table-row">
              ...
              <td class="td-name">
                <div class="agent-name">👤 {{ p.agent_name || 'Noma\'lum Agent' }}</div>
              ...
              <td class="td-amount">
                <span class="amount-val text-amber">{{ formatMoney(p.amount) }} so'm</span>
              </td>
              ...
              <div class="searchable-select-wrap">
                ...
                <div v-if="showAgentDropdown" class="dropdown-options-list">
                  <div
                    ...
                    :class="{ selected: form.agent === ag.id }"
            ...
            <div class="form-group">
              ...
              <div class="searchable-select-wrap">
                ...
                <div v-if="showReturnAgentDropdown" class="dropdown-options-list">
                  <div
                    v-for="ag in filteredReturnAgents"
                    :key="ag.id"
                    class="dropdown-option-item"
                    :class="{ selected: returnForm.agent === ag.id }"
...
async function fetchPayments() {
  loading.value = true
  try {
    const params = { status: 'bonus', page_size: 200 }
...
watch(filterAgentName, () => { fetchPayments() })
...
async function confirmDelete(p) {
  if (!confirm(`Haqiqatan ham #${p.id} raqamli bonus to'lovini o'chirmoqchimisiz?`)) return
  try { await api.delete(`/payments/${p.id}/`); fetchPayments() }
  catch (err) { alert("O'chirishda xatolik yuz berdi") }
}

onMounted(() => { fetchPayments(); fetchEnrollments(); fetchAgents() })
</script>
...
.toolbar-bar { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border-bottom: 1px solid #E5E7EB; gap: 16px; }
...
.dropdown-option-item { padding: 10px 14px; cursor: pointer; border-bottom: 1px solid #F3F4F6; &:hover { background: #FEF3C7; } &.selected { background: #FDE68A; font-weight: 700; } }
