<template>
  <AppLayout>
    ...
    <div class="page-top">
      <div>
        <h2 class="page-main-title">Bank uchun to'lovlar</h2>
      ...
      <button class="btn-primary-action btn-blue" @click="openCreateModal">
        ...
        <span>Bank to'lovini kiritish</span>
      </button>
    </div>
    ...
    <div class="metrics-cards-grid">
      <div class="card-metric card-blue">
        ...
        <div>
          ...
          <h4 class="metric-val text-blue">{{ metrics.count }} ta to'lov</h4>
        </div>
      </div>
    ...
    <div class="table-section-card margin-top">
        ...
        <table class="data-table">
          <thead>
            ...
            <tr class="col-filter-row">
              ...
              <th>
                <div class="col-sort-group">
                  <button type="button" class="col-sort-icon-btn" :class="{ active: paymentDateSort === 'asc' }" title="O'sish tartibida (eskidan yangiga)" @click="setSort('asc')">
                    <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M6 20V4"></path>
                      <path d="M3 8l3-4 3 4"></path>
                      <text x="12" y="10" font-size="7.5" font-family="Arial, sans-serif" font-weight="700" stroke="none" fill="currentColor">9</text>
                  ...
                  <button type="button" class="col-sort-icon-btn" :class="{ active: paymentDateSort === 'desc' }" title="Kamayish tartibida (yangidan eskiga)" @click="setSort('desc')">
                    <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M6 4v16"></path>
                      <path d="M3 16l3 4 3-4"></path>
                      <text x="12" y="10" font-size="7.5" font-family="Arial, sans-serif" font-weight="700" stroke="none" fill="currentColor">9</text>
          ...
          <tbody>
            <tr v-if="payments.length === 0">
              <td colspan="6" class="no-data">Bank statusidagi to'lovlar topilmadi</td>
            </tr>
      ...
      <div class="pagination-bar">
        ...
        <div class="pagination-actions">
          ...
          <label class="page-size-label" for="bank-page-size">Ko'rsatish:</label>
          <div class="select-wrap">
            <select id="bank-page-size" v-model="pageSizeOption" class="page-size-select">
...
// Filtering/sorting/pagination are all applied server-side now (see
// buildParams/fetchPayments) — `payments` only ever holds the current page.
...
// Aggregated in the DB (via PaymentViewSet.totals) over every row matching
// the active filters, not just the current page's slice.
const metrics = ref({ total: 0, count: 0 })
...
// Distinct cashiers, for the "To'lov qabul qiluvchi" filter select. Fetched
// separately (capped at 1000, same pattern as the other finance views) so
// the dropdown still lists every cashier even though `payments` itself now
// only holds one page.
const allBankPayments = ref([])
async function fetchDistinctCashiers() {
  try {
    const res = await api.get('/payments/', { params: { status: 'bank', page_size: 1000 } })
    allBankPayments.value = res.data.results || res.data
  } catch (err) { console.error(err) }
}
const distinctCashiers = computed(() => {
  const map = {}
  allBankPayments.value.forEach(p => {
...
watch([filterMethod, filterCashierId, paymentDateFrom, paymentDateTo], refetch)
watch(filterNotes, debounce(refetch, 400))
...
async function savePayment() {
  ...
  try {
    if (isEditing.value) {
      ...
      await api.post('/payments/', { user: authStore.user?.id, amount: form.value.amount, status: 'bank', method: form.value.method, notes: form.value.notes, branch: branchStore.activeBranchId ?? null })
    }
    closeModal()
    fetchPayments()
    fetchTotals()
...
.col-filter-input:focus, .col-filter-select:focus { border-color: #2563EB; }
...
.col-date-input:focus { border-color: #2563EB; outline: none; }
.btn-clear-date {
  border: none;
  background: #F3F4F6;
  color: #6B7280;
...
.page-size-select:focus { border-color: #2563EB; outline: none; }
.btn-page {
  padding: 6px 14px;
  background: white;
