        ...
        <table v-else class="data-table">
          ...
          <tbody>
            <tr v-for="p in payments" :key="p.id" class="table-row">
              ...
              <td class="td-name">
                <div class="student-name">{{ p.notes || p.student_name || 'Bank Operatsiyasi' }}</div>
              </td>
              <td class="td-amount">
                <span class="amount-val text-blue">{{ formatMoney(p.amount) }} so'm</span>
              </td>
...
async function fetchPayments() {
  loading.value = true
  try {
    const params = { status: 'bank', page_size: 200 }
...
async function confirmDelete(p) {
  if (!confirm(`Haqiqatan ham #${p.id} raqamli bank to'lovini o'chirmoqchimisiz?`)) return
