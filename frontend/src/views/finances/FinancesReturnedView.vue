<template>
  <AppLayout>
    ...
    <div class="page-top">
      ...
      <button class="btn-primary-action btn-orange" @click="openCreateModal">
    ...
    <div class="metrics-cards-grid">
      <div class="card-metric card-red">
        ...
        <div>
          ...
          <h4 class="metric-val text-red">{{ payments.length }} ta to'lov</h4>
        </div>
      </div>
      ...
      <div class="card-metric card-orange font-hero">
        ...
        <div>
          ...
          <h4 class="metric-val text-orange">{{ formatMoney(metrics.total) }} so'm</h4>
        </div>
      </div>
    </div>
    ...
    <div class="table-section-card margin-top">
      <div class="toolbar-bar">
        ...
        <div class="total-count">
          Jami: <strong>{{ totalCount }}</strong> ta qaytarilgan to'lov
        </div>
      </div>
      ...
      <div class="table-container">
        ...
        <div v-else-if="payments.length === 0" class="empty-state">
          <p>Qaytarilgan statusdagi to'lovlar topilmadi</p>
        </div>
        ...
        <table v-else class="data-table">
          ...
          <tbody>
            <tr v-for="p in payments" :key="p.id" class="table-row">
              ...
              <td class="td-amount">
                <span class="amount-val text-red">-{{ formatMoney(p.amount) }} so'm</span>
              </td>
...
async function fetchPayments() {
  loading.value = true
  try {
    const params = { status: 'returned', page_size: 200 }
...
watch(filterStudentName, () => { fetchPayments() })
...
async function confirmDelete(p) {
  if (!confirm(`Haqiqatan ham #${p.id} raqamli qaytarilgan to'lovni o'chirmoqchimisiz?`)) return
  try { await api.delete(`/payments/${p.id}/`); fetchPayments() }
  catch (err) { alert("O'chirishda xatolik yuz berdi") }
}

onMounted(() => { fetchPayments(); fetchEnrollments() })
</script>
...
.dropdown-option-item { padding: 10px 14px; cursor: pointer; border-bottom: 1px solid #F3F4F6; &:hover { background: #FEE2E2; } &.selected { background: #FCA5A5; font-weight: 700; } }
