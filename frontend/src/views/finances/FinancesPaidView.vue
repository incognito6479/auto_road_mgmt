    ...
    <div class="table-section-card margin-top">
      <div class="toolbar-bar">
        <div class="search-box">
          ...
          <input
            ...
            type="text"
            placeholder="Izoh yoki tafsilotlar bo'yicha..."
            class="search-input"
          />
        </div>
      ...
      <div class="table-container">
        ...
        <div v-else-if="payments.length === 0" class="empty-state">
          <p>To'langan statusdagi to'lovlar topilmadi</p>
        </div>
...
watch([filterStudentName, filterMethod], () => { fetchPayments() })
...
async function savePayment() {
  ...
  try {
    if (isEditing.value) {
      ...
      await api.post('/payments/', { user: authStore.user?.id, amount: form.value.amount, status: 'paid', method: form.value.method, notes: form.value.notes })
    }
    closeModal()
    fetchPayments()
