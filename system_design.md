...
- **UI language policy**: every user-facing string in the web app — labels, buttons, placeholders, table headers, page titles, filter options, empty-state and error messages — must be Uzbek only. No English words, and no English gloss in parentheses next to an Uzbek term (e.g. not "Oylik (Salary)"). This applies across all `frontend/src/views` and `frontend/src/components`. English stays confined to code identifiers/comments and the Django admin locale (`/admin/`), which is intentionally English per `AppLanguageMiddleware`. Exceptions (kept as-is, not translated): role names `admin`/`superuser`, and proper nouns/technical terms like `QR` (code) and `Click` (the Uzbek payment provider's brand name).
- **Clickable-to-detail styling convention**: any value that navigates to another record's detail page on click (student/group/agent/user/instructor/coordinator names, etc.) must render **bold, blue (`#2563EB`), underlined**, with a darker blue (`#1D4ED8`) on hover — this is the `.link-value` utility class used across `frontend/src/views` (defined with `!important` on `color`/`font-weight` since it's applied alongside a semantic name class like `.student-name`/`.teacher-name` that would otherwise win the color on source order). Apply this class to any new or edited clickable-detail element; non-clickable variants of the same field (e.g. the `v-else` branch when there's no id to link to) must NOT get this class/styling.
